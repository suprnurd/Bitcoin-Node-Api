  ...
  function express_app(){
    ...
    app.get('*', hasAccess, function(req, res){
      ...
      if(method == 'sendmany' || 
          ...
          method == 'getvotelist'){
        command = specialApiCase(method, params);
      }else{
        command = [{
          method: method,
          params: params
        ...
        if (query_parameters.length == 3) {
          ...
          for(var parameter in query_parameters){
            if(query_parameters.hasOwnProperty(parameter)){
              ...
              if(index == 0 && parameter == 'addr'){
                params.push(param);
              }
  
              if(index == 1 && parameter == 'amt'){
                params.push(param);
              }
  
              if(index == 2 && parameter == 'secret'){
                if (query_parameters[secret] != md5(query_parameters[addr] + query_parameters[amt] + secret_key)){
                  method_name = '';
                  params = [];
                } else {
                  //it works!
